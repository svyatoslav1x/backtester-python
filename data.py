...
class HistoricCSVDataHandler(DataHandler):
    ...
    def _open_convert_csv_files(self, source):
        ...
        for symbol in self.symbol_list:
            ...
            if combined_index is None:
                ...
                # FIX 1: Assign the result of union back to the variable
                combined_index = combined_index.union(self.symbol_data[symbol].index)

            self.latest_symbol_data[symbol] = []

        for symbol in self.symbol_list:
            # FIX 2: Removed method="pad" from reindex, chained .ffill() instead
            self.symbol_dataframe[symbol] = (
                self.symbol_data[symbol].reindex(index=combined_index).ffill()
            )
            self.all_data[symbol] = self.symbol_dataframe[symbol].copy()

            # FIX 3: Replaced fillna(method="pad") with ffill()
            self.symbol_dataframe[symbol].ffill(inplace=True)
    ...
    def _get_new_data(self, symbol):
        for row in self.symbol_data[symbol]:
            # FIX 4: Used .iloc[0] to access value by position instead of label
...
class QuandlDataHandler(DataHandler):
    def __init__(
        self, events, symbol_list, api_key, start_date="2000-01-01", end_date=None
    ):
    ...
    def _load_convert_quandl_data(self):
        ...
        for symbol in self.symbol_list:
            # FIX 2 (Quandl): Replaced reindex method="pad"
            self.symbol_dataframe[symbol] = (
                self.symbol_data[symbol].reindex(index=combined_index).ffill()
            )
            self.all_data[symbol] = self.symbol_dataframe[symbol].copy()

            # FIX 3 (Quandl): Replaced fillna(method="pad")
            self.symbol_dataframe[symbol].ffill(inplace=True)
            self.symbol_data[symbol] = self.symbol_dataframe[symbol].iterrows()

    def _get_new_data(self, symbol):
        for row in self.symbol_data[symbol]:
            # FIX 4 (Quandl): Used .iloc[0]
            yield tuple([symbol, row[0], row[1].iloc[0]])
    ...
    def _get_nasdaq_data(self, symbol):
        self.symbol_data[symbol] = quandl.get(
            "NASDAQOMX/" + symbol, start_date=self.start_date, end_date=self.end_date
        )
        self.symbol_data[symbol].drop(
            columns=["High", "Low", "Total Market Value", "Dividend Market Value"],
            inplace=True,
        )
        self.symbol_data[symbol].columns = ["Close"]
