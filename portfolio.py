...
class NaivePortfolio(Portfolio):
    ...
    def update_timeindex(self, event):
        data = {
            symbol: self.data.get_latest_data(symbol) for symbol in self.symbol_list
        }
        ...
        for symbol in self.symbol_list:
            market_value = (
                self.current_positions[symbol] * data[symbol][0][self.data.price_col]
            )
    ...
    def plot_all(self):
        ...
        # FIX: Save plot instead of showing
        print("Saving performance plot to 'portfolio_performance.png'...")
